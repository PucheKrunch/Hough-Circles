...
def sort_by_x(circles):
    ...
    x = [ str(f"{x[i][0]}    Posición X: {x[i][1]}") for i in range(len(x))]
    x = "\n".join(x)
    return x

def sort_by_y(circles):
    #Insertion sort
    ...
    y = [ str(f"{y[i][0]}    Posición Y: {y[i][1]}") for i in range(len(y))]
    y = "\n".join(y)
    return y
